// Gemini Sidebar Extension - Content Script
...
async function supabaseFetch(method: string, path: string, body?: any): Promise<any> {
...
async function syncToCloud(data: SessionData): Promise<void> {
  const email = getAccountEmail();
  if (email === 'default') return;
  ...
  const cloudRows = await supabaseFetch(
    'GET',
    `questions?user_email=eq.${encodeURIComponent(email)}&page_url=eq.${encodeURIComponent(pageUrl)}&select=id`
  );
  if (cloudRows) {
...
async function syncFromCloud(): Promise<SessionData | null> {
  const email = getAccountEmail();
  if (email === 'default') return null;
  const pageUrl = getSessionId();

  // 메모 가져오기
  const sessions = await supabaseFetch(
    'GET',
    `sessions?user_email=eq.${encodeURIComponent(email)}&page_url=eq.${encodeURIComponent(pageUrl)}&select=memo`
  );

  // 질문 가져오기
  const questions = await supabaseFetch(
    'GET',
    `questions?user_email=eq.${encodeURIComponent(email)}&page_url=eq.${encodeURIComponent(pageUrl)}&select=id,text,bookmarked,created_at&order=created_at.asc`
  );

  if (!sessions && !questions) return null;

  const memo = sessions?.[0]?.memo ?? '';
  const qs: Question[] = (questions ?? []).map((r: any) => ({
    id: r.id,
    text: r.text,
  ...
  if (qs.length === 0 && !memo) return null;
...
async function loadSession(): Promise<SessionData> {
  ...
  const local: SessionData = result[key] ?? { questions: [], memo: '' };
...
// --- State ---
let state: SessionData = { questions: [], memo: '' };
let activeTab: 'questions' | 'bookmarks' | 'memo' = 'questions';
let memoMode: 'write' | 'read' = 'write';
...
let bookmarkCardList: HTMLElement;
let inputArea: HTMLTextAreaElement;
let memoEditor: HTMLTextAreaElement;
let memoPreview: HTMLElement;
let questionsPane: HTMLElement;
let bookmarksPane: HTMLElement;
let memoPane: HTMLElement;
let tabQuestions: HTMLElement;
let tabBookmarks: HTMLElement;
let tabMemo: HTMLElement;
let memoToggleBtn: HTMLElement;
let emptyState: HTMLElement;
let bookmarkEmptyState: HTMLElement;
...
// --- Page Layout ---
function updatePageLayout(width: number, visible: boolean): void {
  if (!pageStyle) {
...
function findGeminiInput(): HTMLElement | null {
  ...
  for (const sel of selectors) {
    ...
    if (el && el.closest('#gemini-sidebar') === null) return el;
  }
  return null;
}

function applyToGemini(text: string): void {
...
// --- Edit Area Helper ---
function createEditCard(q: Question, card: HTMLElement, onDone: () => void): void {
...
function renderBookmarks(): void {
  ...
  if (countEl) countEl.textContent = `${bookmarked.length}개 보관`;
...
function renderMemo(): void {
  if (memoMode === 'write') {
    memoEditor.style.display = 'block';
    memoPreview.style.display = 'none';
    memoEditor.value = state.memo;
    const cb = memoToggleBtn.querySelector('.gs-toggle-checkbox') as HTMLInputElement | null;
...
function confirmDeleteBookmark(id: string): void {
...
function confirmBulkDeleteBookmarks(): void {
  const bookmarked = state.questions.filter((q) => q.bookmarked);
...
async function saveEdit(id: string, newText: string): Promise<void> {
...
function debounceSaveMemo(): void {
...
function switchTab(tab: 'questions' | 'bookmarks' | 'memo'): void {
  ...
  THEMES.forEach((t) => {
    ...
    opt.addEventListener('click', () => {
      ...
      themeDropdown.querySelectorAll('.gs-theme-option').forEach((optEl) => {
        optEl.classList.toggle('gs-theme-current', (optEl as HTMLElement).dataset.theme === t.id);
      });
